...
from welcome import draw_welcome_screen, handle_welcome_events, get_game_status
...
def run():
    game_started, selected_opponent = get_game_status()

    if not game_started:
        draw_welcome_screen(win)
        handle_welcome_events()
    else:
        main(selected_opponent)
