...
def draw_sidebar_gameboards(pre_board, ai_move, candidate_moves=None):
    ...
    square = mini_size // 8  # We now scale the entire 8x8 board into this size

    def draw_mini_full(board, moves_marker=None, chosen_move=None, offset_y=0):
        # draw 8x8 mini board
        rect = pygame.Rect(panel_x, y0 + offset_y, mini_size, mini_size)
        ...
        if moves_marker:
            for mv in moves_marker:
                ...
                pygame.draw.circle(win, (0, 0, 255), (cx, cy), 3)

        # draw arrow for chosen move
        if chosen_move:
            fx, fr = chess.square_file(chosen_move.from_square), 7 - chess.square_rank(chosen_move.from_square)
            tx, tr = chess.square_file(chosen_move.to_square), 7 - chess.square_rank(chosen_move.to_square)
            start = (panel_x + fx * square + square // 2, y0 + offset_y + fr * square + square // 2)
            end = (panel_x + tx * square + square // 2, y0 + offset_y + tr * square + square // 2)
            pygame.draw.line(win, (255, 0, 0), start, end, 2)

    draw_mini_full(pre_board, offset_y=0)
    pygame.font.init()
    font = pygame.font.SysFont("Arial", 14)
    win.blit(font.render("Before", True, (0, 0, 0)), (panel_x, y0 - 18))

    draw_mini_full(pre_board, candidate_moves, ai_move, offset_y=mini_size + gap)
    win.blit(font.render("AI Thinks", True, (0, 0, 0)), (panel_x, y0 + mini_size + gap - 18))
